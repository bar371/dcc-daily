...
Built, tested, and run against the live wiki.
...
**Real run results:** 160 achievement pages + 147 monster pages cached in
`raw/`. `build_dataset.py --report` on that data: achievements 151 pages, 0
unresolved tiers, 1 missing body; monsters 141 pages, 1 unresolved, 10
missing body. Output: 280 entries across B1-B8. The extractor picked up
edge cases the fixtures didn't originally cover — `<blockquote>` bodies,
the wiki's own `{{SpoilH|N}}` spoiler marker, `{{cite|N|..}}`/`{{ref|N|..}}`
templates, the `{{Achievement|...|reward=...}}` infobox, redirect stubs, and
category meta-pages (`Achievement`, `Boss`, ...) — parser and fixtures
updated to match.

**Exit criteria:** 150+ entries ✅, fewer than 10% unresolved tiers ✅ (0%
achievements, 0.7% monsters). Still open: floor ranges in `config/books.json`
verified by you (see `docs/spoilers.md`) — all currently `"verified": false`.
...
`icon-192.png`, `icon-512.png`, `icon-180.png` are generated by
`tools/gen_icons.py` (redraws `icon.svg`'s shapes with Pillow — no SVG
renderer was available in this environment).

**Still to do:** verify the layout in a real browser. Confirm offline works
after a hard reload with the network off.
